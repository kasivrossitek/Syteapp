 ...
 */
public class SyteAnalyticsMonthsFragment extends Fragment
    {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
            {
                mRootView = inflater.inflate(R.layout.fragment_syte_analytics_months, container, false);
                return mRootView;
            }// END onCreateView()
        @Override
        ...
        private void getData()
            {
                mPrgDia.show();
                new Firebase(StaticUtils.ANALYTICS_SYTE_URL).child(mSyteId).child(StaticUtils.ANALYTICS_MONTHLY).orderByKey().limitToFirst(12).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
        ...
        private void mInItData(DataSnapshot dataSnapshot)
            {
                ...
                ArrayList<String> monthsShort = new ArrayList<>(Arrays.asList("Ja", "Fe", "Mr", "Ap", "Ma", "Ju", "Jl","Au","Se","Oc","No","De" ));
                ArrayList<String> monthDates = new ArrayList<>();
                SimpleDateFormat TIME_FORMAT = new SimpleDateFormat("yyyyMMMM");
                SimpleDateFormat TIME_FORMAT1 = new SimpleDateFormat("yy");
                ...
                for(int i =11;i>=0;i--)
                    {
                        ...
                        int MN_OF_YEAR = cal.get(Calendar.MONTH);
                        String month = monthsShort.get(MN_OF_YEAR);
                        Date daysBeforeDate = cal.getTime();
                        String strDate =TIME_FORMAT.format(daysBeforeDate);
                        monthDates.add(strDate);
                        month=month+"'"+TIME_FORMAT1.format(daysBeforeDate);
                        mXAxisLables.add(month);
                        mVisitorData.add(0f);
                    }
                        ...
                        while (iterator.hasNext())
                            {
                                ...
                                if(monthDates.contains(dataSnapshot1.getKey()))
                                    {
                                        int index = monthDates.indexOf(dataSnapshot1.getKey());
